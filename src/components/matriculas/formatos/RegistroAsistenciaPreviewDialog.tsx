...
import RegistroAsistenciaDocument from "./RegistroAsistenciaDocument";
...
const PRINT_STYLES = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .field-cell .fecha-print { display: block !important; }
  @media print { body { padding: 5mm; } .watermark span { color: rgba(0,0,0,0.03); } }
`;

export default function RegistroAsistenciaPreviewDialog({ open, onOpenChange, persona, matricula, curso }: Props) {
  const [fechaAsistencia, setFechaAsistencia] = useState(curso?.fechaInicio || "");
  const documentRef = useRef<HTMLDivElement>(null);
  const isBlocked = !matricula.firmaCapturada || !matricula.autorizacionDatos;
  const pdfFilename = buildPdfFilename("registro-asistencia", persona);
  ...
  const handlePrint = useCallback(() => {
    ...
    // Clone and replace date input with text for print
      ...
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col p-0 gap-0">
        <DialogHeader className="px-6 py-4 border-b shrink-0">
          <div className="flex items-center justify-between pr-8">
            <DialogTitle>Vista Previa — Registro de Asistencia</DialogTitle>
            ...
            <div className="p-2 bg-muted/30 min-h-full">
              <div ref={documentRef} className="shadow-lg rounded border">
                <RegistroAsistenciaDocument
                  ...
                  onFechaChange={setFechaAsistencia}
                />
              </div>
            </div>
          )}
