...
export function ImportarPersonasDialog({ open, onOpenChange }: Props) {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const personas = validRows.map(r => ({
      ...
      const res = await personaService.createBulk(personas as any);
      setResult(res);
      queryClient.invalidateQueries({ queryKey: ['personas'] });
      toast({ title: `${res.created} personas importadas correctamente` });
      logActivity({ action: "importar", module: "personas", description: `Importó ${res.created} persona(s) desde archivo "${fileName}"`, entityType: "persona", metadata: { archivo: fileName, total_filas: rows.length, importadas: res.created, errores: res.errors.length } });
    } catch {
        ...
        {rows.length === 0 && !result ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/50">
              ...
              <div>
                <p className="font-medium">{result.created} personas creadas exitosamente</p>
          ...
          {result ? (
            ...
            <>
              ...
              <Button onClick={handleImport} disabled={validRows.length === 0 || importing}>
                {importing ? 'Importando...' : `Importar ${validRows.length} personas`}
              </Button>
            </>
          ) : (
            <Button variant="outline" onClick={handleClose}>Cancelar</Button>
