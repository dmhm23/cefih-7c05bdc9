/**
 * Constantes UI del catálogo de bloques en el editor SAFA.
 * (Etiquetas, iconos, paleta del catálogo de bloques.)
 */
import {
  Bookmark,
  Heading,
  AlignLeft,
  TextCursorInput,
  CalendarDays,
  Hash,
  CircleDot,
  ChevronDown,
  CheckSquare,
  ListChecks,
  Zap,
  PenTool,
  Heart,
  ClipboardCheck,
  SmilePlus,
  CalendarCheck,
  Mail,
  Minus,
  Paperclip,
  LayoutGrid,
  type LucideIcon,
} from 'lucide-react';
import type { TipoBloque } from './types';

export const BLOQUE_TYPE_LABELS: Partial<Record<TipoBloque, string>> = {
  document_header: 'Encabezado institucional',
  section_title: 'Título de sección',
  heading: 'Encabezado',
  paragraph: 'Párrafo',
  text: 'Campo de texto',
  textarea: 'Texto largo',
  email: 'Correo electrónico',
  date: 'Fecha',
  number: 'Numérico',
  radio: 'Selección única',
  select: 'Select',
  checkbox: 'Checkbox',
  multi_choice: 'Selección múltiple',
  auto_field: 'Automático',
  attendance_by_day: 'Asistencia por día',
  signature_capture: 'Captura de firma',
  health_consent: 'Consentimiento salud',
  evaluation_quiz: 'Evaluación',
  satisfaction_survey: 'Encuesta satisfacción',
  divider: 'Divisor',
  file: 'Archivo adjunto',
};

export const BLOQUE_ICONS: Record<string, LucideIcon> = {
  document_header: LayoutGrid,
  section_title: Bookmark,
  heading: Heading,
  paragraph: AlignLeft,
  text: TextCursorInput,
  textarea: AlignLeft,
  email: Mail,
  date: CalendarDays,
  number: Hash,
  radio: CircleDot,
  select: ChevronDown,
  checkbox: CheckSquare,
  multi_choice: ListChecks,
  auto_field: Zap,
  attendance_by_day: CalendarCheck,
  signature_capture: PenTool,
  health_consent: Heart,
  evaluation_quiz: ClipboardCheck,
  satisfaction_survey: SmilePlus,
  divider: Minus,
  file: Paperclip,
};

export const BLOCK_PALETTE: { type: TipoBloque; label: string }[] = [
  { type: 'document_header', label: 'Encabezado institucional' },
  { type: 'section_title', label: 'Título de sección' },
  { type: 'heading', label: 'Encabezado' },
  { type: 'paragraph', label: 'Párrafo' },
  { type: 'text', label: 'Campo de texto' },
  { type: 'date', label: 'Campo de fecha' },
  { type: 'number', label: 'Campo numérico' },
  { type: 'radio', label: 'Selección única' },
  { type: 'multi_choice', label: 'Selección múltiple' },
  { type: 'select', label: 'Lista desplegable' },
  { type: 'checkbox', label: 'Casilla de verificación' },
  { type: 'auto_field', label: 'Campo automático' },
  { type: 'signature_capture', label: 'Captura de firma' },
  { type: 'health_consent', label: 'Consentimiento salud' },
  { type: 'evaluation_quiz', label: 'Evaluación' },
  { type: 'satisfaction_survey', label: 'Encuesta satisfacción' },
  { type: 'attendance_by_day', label: 'Asistencia por día' },
];

export const COMPLEX_TYPES: TipoBloque[] = [];
