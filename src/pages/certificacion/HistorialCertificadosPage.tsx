...
import { useCertificadosPaginated } from "@/hooks/useCertificados";
...
export default function HistorialCertificadosPage() {
  ...
  const {
    ...
    excepcional: typeof filters.excepcional === 'string' ? filters.excepcional : 'todos',
  });
  ...
  useEffect(() => {
    ...
    if (!wrapper) return;
    const onScroll = () => {
      const remaining = wrapper.scrollHeight - wrapper.scrollTop - wrapper.clientHeight;
      if (remaining < 600) fetchNextPage();
    };
    wrapper.addEventListener('scroll', onScroll, { passive: true });
    return () => wrapper.removeEventListener('scroll', onScroll);
  }, [hasNextPage, isFetchingNextPage, fetchNextPage, certificados.length]);
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          <div ref={containerRef} className="overflow-auto max-h-[calc(100vh-280px)]">
            <Table>
              <TableHeader>
                <TableRow>
          ...
          <p className="text-sm text-muted-foreground">
            Mostrando {certificados.length.toLocaleString('es-CO')} de {total.toLocaleString('es-CO')} certificados
          </p>
        </>
      )}
    </div>
