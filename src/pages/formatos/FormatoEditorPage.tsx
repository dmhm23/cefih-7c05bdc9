...
export default function FormatoEditorPage() {
  ...
  const handleSave = async () => {
    ...
    const data: FormatoFormacionFormData = {
      ...
      activo,
      esAutomatico: false,
      motorRender: existing?.motorRender || 'bloques',
      categoria: existing?.categoria || 'formacion',
      estado: activo ? 'activo' : 'borrador',
      usaEncabezadoInstitucional: existing?.usaEncabezadoInstitucional ?? true,
      htmlTemplate: existing?.htmlTemplate,
      cssTemplate: existing?.cssTemplate,
      encabezadoConfig: existing?.encabezadoConfig,
      plantillaBaseId: existing?.plantillaBaseId,
      tokensUsados: existing?.tokensUsados,
